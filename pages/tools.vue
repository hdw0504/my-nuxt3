...
import type { GoodDetail, Pid, Vid } from '~/types/tb'
...
const propList = ref<[Pid, Vid][]>([])
const skuDict = reactive<Record<string, string>>({})
function initSkuConfig() {
  // 商品id
  goodId.value = goodInfo.value?.item?.itemId
  // 购买数量
  buyNum.value = 1
...
const totalSku = computed(() => {
  const propPath = propList.value.map(([{ pid }, { vid }]) => `${pid}:${vid}`).join(';')
        ...
        <div v-for="(group, i) in goodInfo.skuBase.props" :key="group.pid" class="mb-2">
          ...
          <div class="flex flex-wrap gap-2.5px">
            <p
              ...
              :class="{ active: propList[i]?.[1].vid === item.vid }"
              @click="setPropKey(group, item, i)"
            >
              {{ item.name }}
            </p>
          </div>
